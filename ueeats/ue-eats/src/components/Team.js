...
const Team = () => {
  ...
  // State variables
  const [ratings, setRatings] = useState({});
  const [status, setStatus] = useState(null);  // For status messages (rating submit success/error)
  ...
  const checkLoginStatus = () => {
    ...
    setIsLoggedIn(!!token); // Set login status based on presence of authToken
  };

  // Run on initial render to check login status
  useEffect(() => {
    checkLoginStatus();
  }, []);
  ...
  const handleRatingSubmit = async (itemName) => {
    if (!isLoggedIn) {
      navigate('/loginn');  // Navigate to the login page if not logged in
      return;
    }
    ...
    const ratingData = { itemName, rating };
  ...
  const handleDetailsClick = (itemName) => {
    ...
    navigate(`/${encodeURIComponent(itemName)}`);  // Navigate to the details page for the item
  };
  ...
  const handleOrderClick = () => {
    ...
    navigate('/Contactt');  // Redirect to the contact page to place an order
  };
